...
class CrackerBox(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # naming paths
        name_img = os.path.join(self.data_path, os.path.basename(filename_gt.replace('-box.txt', '.jpg')))
        if not os.path.exists(name_img):
          name_imgalt = os.path.join(self.data_path, os.path.basename(filename_gt.replace('.txt', '.jpg')))
          name_img = name_imgalt
        ...
        if grid_x < self.yolo_grid_num and grid_y < self.yolo_grid_num:
          cx_change = (cx - grid_x * self.yolo_grid_size) / self.yolo_grid_size
          cy_change = (cy - grid_y * self.yolo_grid_size) / self.yolo_grid_size
          w_change = w / self.yolo_image_size
          h_change = h / self.yolo_image_size

          gt_box_blob[:, grid_y, grid_x] = torch.tensor([cx_change, cy_change, w_change, h_change, 1.0])
