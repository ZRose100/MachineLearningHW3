...
def compute_loss(output, pred_box, gt_box, gt_mask, num_boxes, num_classes, grid_size, image_size):
    ...
    loss_x = weight_coord * torch.sum(box_mask * torch.pow(gt_box[:, 0:1] - output[:, 0:1], 2.0))
    loss_y = weight_coord * torch.sum(box_mask * torch.pow(gt_box[:, 1:2] - output[:, 1:2], 2.0))
    loss_w = weight_coord * torch.sum(box_mask * torch.pow(torch.sqrt(gt_box[:, 2:3]) - torch.sqrt(output[:, 2:3]), 2.0))
    loss_h = weight_coord * torch.sum(box_mask * torch.pow(torch.sqrt(gt_box[:, 3:4]) - torch.sqrt(output[:, 3:4]), 2.0))
    loss_obj = torch.sum(box_mask * torch.pow(box_confidence - output[:, 4:5*num_boxes:5], 2.0))
    loss_noobj = weight_noobj * torch.sum((1 - box_mask) * torch.pow(output[:, 4:5*num_boxes:5], 2.0))
    loss_cls = torch.sum(box_mask * torch.pow(gt_box[:, 4:] - output[:, 5*num_boxes:], 2.0))
